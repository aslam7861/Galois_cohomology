...
\begin{definition}
Let $L/K$ be a finite abelian field extension. A modulus $\mathfrak{m}$ of $K$ is said to be admissible for an abelian extension $L/K$ if and only if (almost) all primes $ \mathfrak{p} \in \mathfrak{R}_{\mathfrak{m}}$ of $K$ totally split in $L$.      \\
OR\\
There exist a modulus $\mathfrak{m}$ such that $\Gal(L/K)\cong Cl_{\mathfrak{m}}$.
...
\subsection{Shafarevich-Weil theorem }
