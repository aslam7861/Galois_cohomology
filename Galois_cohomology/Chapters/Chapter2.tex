...
\REQUIRE   $L/K $ be ramified $p$-adic field extension and $a=\omega\left(1+p*Random(k)\right)$.
\ENSURE  $\beta \in L $ scuh that $Norm(\beta) =a$.

\STATE  	Compute the residue field extension  $\mathbb{F}_q$ and $\mathbb{F}_p$ of $L$ and $K$ resp.
 \STATE Try to find $\epsilon$ in $\mathbb{F}_q$ such that $\epsilon^e := \gamma$, where $e$ is the ramification idex of $L/K$.
...
\STATE Compute the preimage of $g^{ze}$ in the ring of integers of $L$ and let us denote it by $\tilde{\beta}$.
\end{algorithmic}
\end{algorithm} 	
...
Let $\N: L\rightarrow K$ be the norm map. In unramified extension $\N_{\mkern 2mu \vrule height 2ex\mkern2mu U_{L}}$ is surjective to $U_{K}$. That is for every $a \in U_{K}$ we find $b \in U_{L}$ such that
\begin{equation}
   \N(b)= a.
\end{equation}
%That is, $\N(b)= a.
...
%  %\hline
%  \label{tab:table1}
%  \begin{tabular}{|a|b|c|d|e|f|g|h|i|}  %{ |l|s|r|t|d|e|f|g|h| } %{|a|b|c|d|e|f|g|h|i|}   %{l|s|r|t|d|e|f|g|h} 	
%  	%\toprule
%  	\hline
